...
export class RecordingQueries {
  ...
  public async selectRecordingDevices(recordingId: string) {
    ...
    const deviceSettings = JSON.parse(devicesSettings.devices) as DeviceInfoSavedType[];
   ...
   */
  public async insertRecording(recording: { name: string; has_data: 0 | 1; description?: string }) {
    const ts = Date.now();
   ...
   */
  public async updateRecordingDevices(devicesSettings: DeviceInfoSavedType[], recordingId: string) {
    const ts = Date.now();
  ...
  public async insertTestDeviceData(data: Buffer) {
    ...
    if (testBuff.length === 50) {
      const dataCompressed = serialize(testBuff) as Buffer;
