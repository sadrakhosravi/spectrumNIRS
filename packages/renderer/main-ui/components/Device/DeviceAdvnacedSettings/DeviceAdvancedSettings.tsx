...
// View Models
import { appRouterVM, deviceManagerVM } from '@viewmodels/VMStore';
...
export const DeviceAdvancedSettings = ({ deviceName, open, closeSetter }: DeviceSelectorType) => {
  const [deviceSettings, setDeviceSettings] = React.useState<DeviceInfoType | null>(null);

  React.useEffect(() => {
    (async () => {
      const settings = await deviceManagerVM.getDeviceSettings(deviceName);
