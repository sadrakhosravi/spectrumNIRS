...
export class DownSampler {
  ...
  constructor(
    ...
    if (downsampledRate > currSamplingRate) {
      throw new Error('Downsampled rate cannot be greater than the device sampling rate');
    }
  ...
  public processData(data: any) {
    ...
    const totalWindows = data[channels[0]][leds[0]].length / this.downsamplingFactor;
    ...
    for (let i = 0; i < channels.length; i++) {
      ...
      for (let j = 0; j < leds.length; j++) {
        ...
        for (let k = 0; k < totalWindows; k++) {
          ...
          const avg =
            windowedData.reduce((prev: any, curr: any) => prev + curr) / this.downsamplingFactor;
