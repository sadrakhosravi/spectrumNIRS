    ...
    function V5Calculation() {
        ...
        this.processRawData = function (dataBatch, batchSize) {
            ...
            for (var i = 0; i < batchSize; i += 1) {
                ...
                var rawPDValues = dataBatch.slice(arrayIndex, arrayIndex + NUM_OF_RAW_PD_VALUES);
                arrayIndex += NUM_OF_RAW_PD_VALUES;
                var LEDIntValues = dataBatch.slice(arrayIndex, arrayIndex + _this.NUM_OF_LEDs);
        ...
        this.processDbData = function (dataBatch) {
            var batchSize = dataBatch.length;
            var calculatedData = new Array(batchSize);
            for (var i = 0; i < batchSize; i += 1) {
                // Prepare arrays for calculation
                var rawPDValues = dataBatch[i].PDRawData;
                var LEDIntValues = dataBatch[i].LEDIntensities;
                // Calculate values
                var dataArray = _this.calcHemodynamics(rawPDValues);
                dataArray.push(_this.calcTOI(rawPDValues, LEDIntValues));
