...
export class RegisterDevices {
  ...
  public static async registerInDB() {
    ...
    const allDBDevice =
      await ServiceManager.dbConnection.deviceQueries.selectAllDevices();

    // If the device does not exist, register it.
    devices.forEach(async (device) => {
      const isDeviceInDb = allDBDevice.find(
        (DBDevice) => DBDevice.name === device.name
      );
      ...
      if (!isDeviceInDb) {
        ...
        await ServiceManager.dbConnection.deviceQueries.insertDevice(
          deviceToAdd
        );
      }
    });
