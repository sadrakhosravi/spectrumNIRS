...
export class DeviceModel {
  ...
  // Constructor
  constructor(id: number, name: string, workerCreator?: any) {
    this.id = id;
    this.name = name;
  ...
  //   const activeConfig =
  //     await ServiceManager.dbConnection.deviceQueries.selectActiveDeviceConfig(
  //       this.id
  //     );
  //   // If no configs found, insert the default config
  //   if (!activeConfig) {
  //     const defaultConfig = await this.wrappedWorker.getDefaultConfig();
  //     await ServiceManager.dbConnection.deviceQueries.insertConfig(
  //       defaultConfig,
  //       this.id,
  //       true
  //     );
  ...
  public async applySavedConfig() {
    await this.wrappedWorker.setDeviceConfig(
      this.deviceConfig?.settings as IDeviceConfigParsed
    );
  }
