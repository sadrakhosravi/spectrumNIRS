...
class NIRSCalculation implements IDeviceCalculation {
  ...
  protected calcTOI = (sample: Float32Array) => {
    ...
    L_coefreq[0] *= this.LEDCorrectionFactor[0] * this.ADCMaxVal;
    L_coefreq[1] *= this.LEDCorrectionFactor[1] * this.ADCMaxVal;
    L_coefreq[2] *= this.LEDCorrectionFactor[3] * this.ADCMaxVal;
    L_coefreq[3] *= this.LEDCorrectionFactor[4] * this.ADCMaxVal;
    ...
    for (let i = 0; i < this.NUM_OF_LEDs; i++) {
      ...
      if (i !== this.NUM_OF_LEDs - 1) OD_Test[i] = Math.log(L_coefreq[i]) * -1;
