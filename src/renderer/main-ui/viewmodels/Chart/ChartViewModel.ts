...
export class ChartViewModel {
   ...
   */
  @observable public parentContainerStyle: CSSProperties = {
    overflow: 'hidden',
  };
  /**
   ...
   */
  @action public addSeries(
    chartId: string,
    seriesName: string,
    seriesColor?: string
  ) {
    const chart = this.charts.find(
      (chart) => chart.dashboardChart.getId() === chartId
    );
    ...
    const series = chart.dashboardChart.addLineSeries(
      seriesName || 'No Name',
      color || '#fff'
    );
  ...
  private handleReactions() {
    ...
    const chartLengthReaction = reaction(
      ...
      () => {
        ...
        const interval = this.charts[0].dashboardChart.chart
          .getDefaultAxisX()
          .getInterval();
        const allCharts = this.charts.map((chart) => {
          chart.dashboardChart.chart
            .getDefaultAxisX()
            .setInterval(interval.start, interval.end);
          return chart.dashboardChart.chart;
        });

        this.xAxisSynchronizedHandler =
          this.model.synchronizeChartXAxes(allCharts);
    ...
    const chartMaximizedReaction = reaction(
      ...
      () => {
        if (this.isChannelMaximized) {
          ...
          this.parentContainerStyle = {
            overflow: this.charts.length > 12 ? 'auto' : 'hidden',
          };
        }
      }
    );
