...
export const DeviceSettings = observer(({ device }: DeviceSettingsType) => {
  ...
  React.useEffect(() => {
    ...
    const clearSpan = () =>
      (timeoutRef = setTimeout(() => (statusSpan.innerText = ''), 3500));

    ipcRenderer.on(
      ReaderChannels.START,
      (_event, status: boolean | undefined) => {
        ...
        switch (status) {
          ...
          case false:
            statusSpan.innerText =
              'Failed to update settings on device! Please try again.';
            statusSpan.style.color = 'red';
            clearSpan();
            break;
