...
export const ScreenView = ({ enableWidgets }: ScreenViewType) => {
  ...
  const resizeSplitView = React.useCallback(
    (e: React.MouseEvent<HTMLDivElement, MouseEvent>) => {
      ...
      const widgetView = document.getElementById(
        widgetViewId
      ) as HTMLDivElement;
        ...
        if (pointerXDiff < 0) {
          if (pointerXDiff > -200) {
            contentView.style.width = `${
              contentViewInitialWidth - Math.abs(pointerXDiff)
            }px`;
            widgetView.style.width = `${
              widgetViewInitialWidth + Math.abs(pointerXDiff)
            }px`;
