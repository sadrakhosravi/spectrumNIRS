...
export const ChannelSettings = observer(
  ({ parentRef, chart, closeSetter }: ChannelSettingsType) => {
    ...
    React.useEffect(() => {
      if (chart && chart.series.length !== 0) {
        const interval = chart.dashboardChart.chart
          .getDefaultAxisY()
          .getInterval();
    ...
    const handleGainValueChange = React.useCallback(
    ...
    const handleYMinIntervalChange = React.useCallback(
      (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        if (isNaN(value)) return;
        const currentInterval = chart.dashboardChart.chart
          .getDefaultAxisY()
          .getInterval();

        chart.dashboardChart.chart
          .getDefaultAxisY()
          .setInterval(value, currentInterval.end);
    ...
    const handleYMaxIntervalChange = React.useCallback(
      (e: React.ChangeEvent<HTMLInputElement>) => {
        ...
        chart.dashboardChart.chart
          .getDefaultAxisY()
          .setInterval(currentInterval.start, value);
        setIntervalUpdate(!intervalUpdate);
      },
      [intervalUpdate]
    );
