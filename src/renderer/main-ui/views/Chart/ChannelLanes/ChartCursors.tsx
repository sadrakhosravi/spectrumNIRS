...
// Styles
import styles from './channelLanes.module.scss';
...
export const ChartCursors = observer(() => {
  ...
  const createCursorsMaximized = React.useCallback(() => {
    const chart = chartVM.charts.find(
      (chart) => chart.id === chartVM.isChannelMaximized
    );
    if (!chart) return;
  ...
  return (
    <div
      ...
      onMouseEnter={
        chartVM.isChannelMaximized ? createCursorsMaximized : createCursors
      }
        ...
        (cursor, i) =>
          cursor.y !== 0 && (
            <div
              ...
              className={`${styles.CursorItem} ${
                cursor.x > 600 && styles.CursorItemSpanReversed
              }`}
              style={{
