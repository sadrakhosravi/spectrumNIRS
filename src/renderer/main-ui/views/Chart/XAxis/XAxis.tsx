...
export const XAxis = observer(() => {
  let xAxisVM: XAxisChartViewModel = React.useMemo(
    () => new XAxisChartViewModel(),
    []
  );
  ...
  return (
    <>
      <div className={styles.XAxisLeft}>
        <span>Time Div</span>
        <Listbox
          options={xAxisVM.divisions}
          value={xAxisVM.timeDiv}
          setter={xAxisVM.setTimeDiv}
        />
        <IconOnlyButton
          ...
          tooltipText={
            xAxisVM.isLocked ? 'Restore Axis Scrolling' : 'Stop Axis Scrolling'
          }
        />
      </div>
