...
class V5Calculation {
  ...
  public processRawData = (
    ...
    for (let i = 0; i < batchSize; i += 1) {
      ...
      dataArray[3] = this.calcTOI(rawPDValues, LEDIntValues);
  ...
  public calcHemodynamics = (rawPDValues: number[]) => {
    ...
    const THb = O2Hb + HHb;
    const calculatedData = new Float32Array([O2Hb, HHb, THb, 0]); // The last value (0) will be filled by TOI
    return calculatedData;
  };
   ...
   */
  private calcTOI = (rawPDValues: number[], LEDIntValues: number[]) => {
